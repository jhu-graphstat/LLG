...
Note that $\hat{P}$ appears to better estimate the true probability matrix $P$, especially for edges within the two hemispheres, in the upper left and corresponding lower right block. And by calculating the mean squared error based on this sample, we can also see that $\hat{P}$ (with mean squared error equal 0.017) outperforms $\bar{A}$ (with mean squared error equal 0.027).
}
