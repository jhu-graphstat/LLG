...
Now take a sample of size $M=5$ in the CoRR dataset example with Desikan atlases mentioned above, then we calculate the entry-wise sample mean $\bar{A}$ and plot it in the top middle of Fig.~\ref{fig:Matrix_desikan_m5}. Darker pixels indicate a higher probability of an edge between the given vertices.
We can see $\bar{A}$ gives a fair estimate of $P$. However, since the sample size is small, it has high variances. On the other hand, the estimator $\hat{P}$ we will propose in this paper has a finer gradient of values which in this case leads to a more accurate estimate. We plot the heat map of $\hat{P}$ in the top right of Fig.~\ref{fig:Matrix_desikan_m5}.
By calculating the mean squared error based on this sample, we can see that $\hat{P}$ (with mean squared error equals $0.017$) outperforms $\bar{A}$ (with mean squared error equals $0.027$).
In order to see where the improvements are clearly, in the upper triangular of the heat maps for $\bar{A}$ and $\hat{P}$, we highlight the edges (79 edges highlighted for $\bar{A}$ and 20 for $\hat{P}$) which have absolute estimation error larger than $0.4$.
The smoothing effect is much more obvious when we decrease the sample size from $M = 5$ to $M = 1$.
In the lower level of Fig.\ref{fig:Matrix_desikan_m5}, we plot three heat maps in the similar way based on sample size $M = 1$. Since there is only one observed graph, $\bar{A}$ has edges with binary weights. However, $\hat{P}$ smooth the estimate (with mean squared error equals $0.041$), which leads to a much better estimate compared to $\bar{A}$ (with mean squared error equals $0.094$). Similarly, we use the same absolute estimation error threshold $0.4$ and highlight 471 edges for $\bar{A}$ and 168 edges for $\hat{P}$.
